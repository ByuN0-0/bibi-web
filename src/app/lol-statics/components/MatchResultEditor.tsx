...
import type {DataDragonAssetKind, LolAssetRef, MatchObjectives, MatchResult, MatchResultParticipant, MatchTeam, PlayerProfile} from "@/lib/lol/types";
...
type PickerState = {kind: DataDragonAssetKind; label: string; current: LolAssetRef | null; nullable: boolean; apply: (asset: LolAssetRef | null) => void};
...
export default function MatchResultEditor({result, players}: {result: MatchResult; players: PlayerProfile[]}) {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      await validateDraftAssets(draft);
      ...
      const response = await fetch(`/api/lol-statics/match-results/${encodeURIComponent(result.matchResultId)}`, {
        ...
        body: JSON.stringify({revision: result.revision, winner: draft.winner, playedOn: draft.playedOn, durationSeconds: draft.durationSeconds, ddragonVersion: draft.ddragonVersion, teamStats, participants: draft.participants}),
      });
...
function ParticipantEditor({participant, index, changed, players, version, onChange, onPick}: {participant: MatchResultParticipant; index: number; changed: boolean; players: PlayerProfile[]; version: string; onChange: (patch: Partial<MatchResultParticipant>) => void; onPick: (state: PickerState) => void}) {
  function pick(kind: DataDragonAssetKind, label: string, current: LolAssetRef | null, nullable: boolean, apply: (asset: LolAssetRef | null) => void) { onPick({kind, label, current, nullable, apply}); }
  return <article className={`rounded-xl border bg-white p-4 ${changed ? "border-[var(--primary)] shadow-[inset_3px_0_0_var(--primary)]" : "border-[var(--hairline-soft)]"}`}>
    <div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><LolIcon asset={participant.champion} version={version} size={42} /><div><p className="font-bold">{participant.observedName}</p><p className="text-[10px] text-[var(--muted)]">{participant.guest ? "게스트" : "등록 선수"} · {participant.champion.name}</p></div></div>{changed && <span className="rounded-full bg-[var(--primary-soft)] px-2 py-1 text-[10px] font-bold text-[var(--primary)]">수정됨</span>}</div>
    <div className="grid gap-3 sm:grid-cols-2"><SelectField label="등록 선수 연결" value={participant.discordUserId ?? ""} onChange={(value) => onChange({discordUserId: value || null, guest: !value})}><option value="">게스트</option>{players.map((player) => <option key={player.discordUserId} value={player.discordUserId}>{player.displayName} · {player.riotGameName}#{player.riotTagLine}</option>)}</SelectField><InputField label="점수판 이름" value={participant.observedName} onChange={(value) => onChange({observedName: value})} /></div>
...
function AssetPicker({version, state, onClose}: {version: string; state: PickerState; onClose: () => void}) {
  ...
  const filtered = useMemo(() => {const normalized = query.trim().toLocaleLowerCase("ko-KR"); return normalized ? assets.filter((asset) => `${asset.name} ${asset.id}`.toLocaleLowerCase("ko-KR").includes(normalized)).slice(0, 80) : assets.slice(0, 80);}, [assets, query]);
