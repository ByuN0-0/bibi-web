...
import {MAX_LEVEL_BY_ROLE, ROLE_LABEL, ROLES} from "@/lib/lol/types";
  ...
  return (
    <form onSubmit={submit} className="space-y-6 pb-24">
      ...
      <section className="surface-card grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4 sm:p-6">
        ...
        <DurationField value={draft.durationSeconds} onChange={(value) => setDraft({...draft, durationSeconds: value})} />
...
function ParticipantEditor({participant, index, changed, players, version, reviewIssues, onChange, onPick}: {participant: MatchResultParticipant; index: number; changed: boolean; players: PlayerProfile[]; version: string; reviewIssues: MatchReviewIssue[]; onChange: (patch: Partial<MatchResultParticipant>) => void; onPick: (state: PickerState) => void}) {
  ...
  return <article className={`rounded-xl border bg-white p-4 ${changed ? "border-[var(--primary)] shadow-[inset_3px_0_0_var(--primary)]" : "border-[var(--hairline-soft)]"}`}>
    ...
    <div className="mt-3 grid grid-cols-[92px_repeat(3,minmax(0,1fr))] gap-2"><NumberField compact label="레벨" value={participant.level} min={1} max={MAX_LEVEL_BY_ROLE[participant.role]} issue={issue("level")} onChange={(value) => onChange({level: value})} /><NumberField compact label="킬" value={participant.kills} min={0} onChange={(value) => onChange({kills: value})} /><NumberField compact label="데스" value={participant.deaths} min={0} onChange={(value) => onChange({deaths: value})} /><NumberField compact label="어시스트" value={participant.assists} min={0} onChange={(value) => onChange({assists: value})} /></div>
...
function DurationField({value, onChange}: {value: number; onChange: (value: number) => void}) {
  ...
  const update = (nextMinutes: number, nextSeconds: number) => onChange(Math.max(1, nextMinutes * 60 + nextSeconds));
  return <fieldset><legend className="text-[10px] font-medium text-[var(--muted)]">진행 시간</legend><div className="grid grid-cols-2 gap-2"><label className="relative"><span className="sr-only">분</span><input type="number" required min={0} value={minutes} onChange={(event) => update(Math.max(0, Number(event.target.value) || 0), seconds)} className="form-control pr-8 text-sm" /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-[var(--muted)]">분</span></label><label className="relative"><span className="sr-only">초</span><input type="number" required min={0} max={59} value={seconds} onChange={(event) => update(minutes, Math.min(59, Math.max(0, Number(event.target.value) || 0)))} className="form-control pr-8 text-sm" /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-[var(--muted)]">초</span></label></div></fieldset>;
}
