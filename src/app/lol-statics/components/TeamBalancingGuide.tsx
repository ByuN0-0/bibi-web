  ...
  return (
    <section id={id} className="mt-3 rounded-xl border border-[var(--hairline)] bg-[var(--surface-soft)] p-4 sm:p-5" aria-labelledby={`${id}-title`}>
      ...
      <div className="mt-4 rounded-lg border border-[var(--hairline-soft)] bg-white p-4">
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted)]">라인 우세는 탑·정글·미드와 원딜·서포터를 합친 봇 듀오까지 네 전장으로 비교하며, 3점 이하는 동률로 봐요. 우세 수 차이가 같으면 최고 선호 퍼센트의 라인에 배정된 선수가 많은 조합, 그다음 동률 전장이 많은 조합을 선택합니다. 포지션 선호 위반값은 1 - 선호도/100으로 계산하고 0% 라인은 가능한 한 먼저 피합니다. 최근 같은 팀 반복은 최근 10번의 확정 편성을 사용해요.</p>
      </div>
