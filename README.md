...
10회로 제한합니다. 선수 관리 화면의 전적 갱신 탭은 웹 서버가 Riot API를
계정별로 직접 호출합니다. SODA의 전역 잠금으로 한 번에 한 계정만 처리하고
이전 갱신 시작 후 2분이 지나야 다음 계정을 시작할 수 있습니다. 최초 갱신만
Riot ID로 계정을 찾고, 이후에는 이름 변경과 무관한 PUUID로 조회합니다.

웹 갱신 함수는 최대 300초로 설정하고 5개 큐의 최근 경기 최대 30개와
타임라인을 수집합니다. Discord 봇의 수동 및 04시 정기 갱신은 사용하지 않습니다.
